...
class Config:
    ...
    USE_LLM_VERIFICATION = True      # 是否启用 LLM／DeepSeek 核验增强模块
    DEEPSEEK_API_KEY = os.getenv("DEEPSEEK_API_KEY", "")  # 替换为你自己的 DeepSeek API KEY
...
def generate_report_filename(file_path: str):
    """根据【输入文件名】生成报告文件名"""
    file_stem = Path(file_path).stem
    safe = re.sub(r"[^\w\-]", "_", file_stem)
    os.makedirs(Config.OUTPUT_DIR, exist_ok=True)
    return os.path.join(Config.OUTPUT_DIR, f"certificate_integrity_{safe}.md")


# ========== DeepSeek 调用模块 ==========
def verify_with_deepseek(fields: dict, cert_no: str) -> str:
    """
    ...
    try:
        resp = requests.post(f"{Config.DEEPSEEK_API_BASE}/chat/completions", json=payload, headers=headers, timeout=30)
...
# ========== 核验证书完整性主函数 ==========
def check_certificate_integrity(json_file: str):
    # 1. 读取 JSON
    ...
    if is_cnas not in ["是", "Yes", "TRUE"]:
        ...
        # 保存并返回
        report_path = generate_report_filename(json_file)
    ...
    client_name = normalize(props.get("委托单位名称") or props.get("客户名称"))
    ...
    if Config.USE_LLM_VERIFICATION:
        ...
        # 将上面准备好的字典传给 LLM
        llm_feedback = verify_with_deepseek(fields_to_check, cert_no)
