...
# =============================
class Config:
    HUAWEI_EXCEL = "./data/华为建议校准周期.xlsx"  # Excel 文件路径
    DB_DIR = "./vector_db/huawei_cycle"  # 向量数据库路径
...
# Excel 解析函数（增强版）
# =============================
def parse_huawei_cycle_excel(excel_path: str) -> List[Dict[str, Any]]:
    """
    解析“华为-建议校准周期.xlsx”
    优化：重点强调【型号规格】与【依据】字段，增强语义检索能力。
    """
    ...
    for _, row in df.iterrows():
        rec = {col: str(row[col]) for col in df.columns}

        # 🌟 加强语义检索文本（型号 + 依据 + 建议周期）
        key_text = f"型号：{row.get('型号规格', '')}；依据：{row.get('依据', '')}；建议周期：{row.get('建议校准周期', '')}"
        full_text = "；".join([f"{k}：{v}" for k, v in rec.items() if v and k != '__text'])
        rec["__text"] = key_text + "；" + full_text
...
# 构建节点
# =============================
def create_huawei_cycle_nodes(records: List[Dict[str, Any]]) -> List[TextNode]:
    nodes = []
...
# =============================
def main():
    print("🚀 启动【华为-校准周期向量数据库】增强版系统...\n")

    # ---------- 模型加载 ----------
    embed_model, llm = init_models()

    # ---------- Excel 解析 ----------
    if not Path(Config.HUAWEI_EXCEL).exists():
        print(f"❌ 未找到 Excel 文件：{Config.HUAWEI_EXCEL}")
        return

    records = parse_huawei_cycle_excel(Config.HUAWEI_EXCEL)
    nodes = create_huawei_cycle_nodes(records)

    # ---------- 构建/加载向量库 ----------
    index = init_vector_store(nodes, embed_model, Config.DB_DIR, "huawei_cycle_data")
    retriever = index.as_retriever(similarity_top_k=Config.TOP_K)
    response_synthesizer = get_response_synthesizer(verbose=True)

    print("\n✅ 系统已就绪！输入检索内容（输入 q 退出）")
