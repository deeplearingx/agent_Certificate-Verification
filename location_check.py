...
class Config:
    ...
    CNAS_DB_DIR = _app.cnas_db_dir
    CNAS_COLLECTION = _app.cnas_collection
    ADDR_DB_DIR = _app.address_db_dir
    ADDR_COLLECTION = _app.address_collection
    ...
    # ========== LLM增强开关 ==========
    USE_LLM_LOCATION_CHECK = _app.use_llm_location_check
    API_KEY = _app.api_key or os.getenv("DEEPSEEK_API_KEY", "")
    API_BASE = _app.api_base
    MODEL = _app.model
    LLM_TEMPERATURE = _app.llm_temperature
    LLM_MAX_TOKENS = _app.llm_max_tokens
...
def init_llm(cfg: Config):
    ...
    return create_openai_like_client(
        ...
        api_base=getattr(cfg, "API_BASE", "https://api.deepseek.com/v1"),
        api_key=getattr(cfg, "API_KEY", os.getenv("DEEPSEEK_API_KEY", "")),
        temperature=getattr(cfg, "TEMPERATURE", 0.0),
        max_tokens=getattr(cfg, "MAX_TOKENS", 256),
    )
...
def search_instruments_by_basis_code(
    ...
    coll = get_collection(app_config.cnas_db_dir, app_config.cnas_collection)
...
def search_address_in_db(
    ...
    app_config = get_app_config()
    coll = get_collection(app_config.address_db_dir, app_config.address_collection)
...
def verify_calibration_location(
    ...
    if has_star:
        ...
        if app_config.use_llm_location_check:
            llm = init_llm(cfg)
...
def check_location(
    ...
    """
    校准地点核验入口（供 Streamlit 调用）
    - embedder_obj: 外部传入的 SentenceTransformer（推荐：复用全局缓存）
    ...
    if embedder_obj is None:
        raise ValueError(
            "check_location: embedder_obj 不能为空，"
            "请从 Streamlit 传入 shared_embedder（已禁止模块内部加载 embedding 模型）"
        )
    ...
    # 🛑 检查点：加载模型后
    if stop_event is not None and stop_event.is_set():
        return "⚠️ 任务已终止（校准地点核验：模型加载后）"

    basis_details: List[Dict[str, Any]] = []
    for criterion in criteria_list:
        # 🛑 检查点：每条依据前
        if stop_event is not None and stop_event.is_set():
            return "⚠️ 任务已终止（校准地点核验：CNAS 检索阶段）"
