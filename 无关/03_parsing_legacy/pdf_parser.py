...
BASE_DIR = APP_CONFIG.local_pdf_dir
OUT_DIR = APP_CONFIG.local_json_dir
...
client = create_openai_client(api_key=API_KEY, api_base=API_BASE)
...
def extract_text_by_page(pdf_path):
    ...
    return pages_data
#清洗函数，标准化提取校准依据
def clean_calibration_basis_list(basis_list):
    """
    清洗“校准依据”：
    - 优先保留 '规程代号 + 可选年份' 和其后规程名称
    - 去掉冒号/分号后面的量程/参数说明（常见为 : 或 ： 后面一长串）
    - 输入输出都是 list[str]
    """
    if not basis_list:
        return basis_list
    ...
    cleaned = []
    for item in basis_list if isinstance(basis_list, list) else [basis_list]:
        s = str(item).strip()
        if not s:
            continue

        # 1) 先截断：遇到中文冒号/英文冒号，只保留前半段（规程名通常在冒号前）
        #    例如：GJB 7691-2012 ...：Voltage:... -> 保留 GJB 7691-2012 ...
        s = re.split(r"[：:]", s, maxsplit=1)[0].strip()

        # 2) 再做一次温和截断：如果后面还跟了大量分号参数，也可以切掉
        #    （可选）如果你希望保留完整规程名称但不保留“;xxx;yyy”
        s = re.split(r"[；;]\s*(Voltage|Time|Rise|Frequency|Input|Calibration)\b", s, maxsplit=1, flags=re.IGNORECASE)[0].strip()

        # 3) 提取“规程代号本体”用于标准化（可选）
        #    若你只想保留代号，把 return 改成只返回 code 即可
        m = re.search(r"\b(JJ[GF]|GJB)\s*\d+(?:\s*-\s*\d{4})?\b", s, re.IGNORECASE)
        if m:
            # 标准化空格：GJB 7691-2012 -> GJB 7691-2012（保留一个空格更易读）
            code = re.sub(r"\s+", " ", m.group(0)).strip().upper()
            # 如果你只想要代号：cleaned.append(code); continue
...
def call_llm(prompt):
    ...
    local_client = create_openai_client(api_key=API_KEY, api_base=API_BASE)
...
#  对外暴露的核心入口函数
# ==========================================
def run_pdf_parsing(pdf_filename, base_dir, out_dir, api_key, stop_event=None):
    """
    供主控脚本调用
    Args:
        stop_event: threading.Event 对象，用于接收停止信号
    """
    global API_KEY, client
    API_KEY = api_key

    # 建议加上错误处理，防止 OpenAI 初始化失败
    try:
        client = create_openai_client(api_key=API_KEY, api_base=API_BASE)
    except Exception as e:
        print(f"❌ [Parser] OpenAI 客户端初始化失败: {e}")
        return None

    pdf_path = str(Path(base_dir) / pdf_filename)
    if not os.path.exists(pdf_path):
        print(f"❌ [Parser] 找不到PDF文件: {pdf_path}")
        return None

    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

    print(f"📘 [Parser] 正在读取 PDF: {pdf_path}")
    ...
    print(f"📄 [Parser] 共 {len(pages)} 页，开始并发解析...")

    # ✅ 核心修改：不在 as_completed 阶段直接合并元数据，先按页收集
    ...
    try:
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            # 使用 as_completed 迭代完成的结果
            for future in as_completed(future_to_page):

                # 1. 【核心】每处理完一个结果，先检查是否按下了停止按钮
                if stop_event and stop_event.is_set():
                    print("🛑 [Parser] 接到终止指令，正在清理线程池...")

                    # 强制取消所有排队中还没开始的任务 (Python 3.9+)
                ...
                try:
                    ...
                    print(f"✅ [Done] 第 {page_num} 页解析完成")

                except Exception as e:
                    print(f"⚠️ [Parser] 第 {future_to_page[future]} 页解析出错: {e}")
                    continue

    except Exception as e:
        print(f"❌ [Parser] 线程池异常: {e}")
        return None
    # === 关键修改区域 End ===
    ...
    # ✅ 核心修改：按页码顺序合并元数据（保证温湿度“第一次/第二次出现”语义稳定）
    final_metadata_props = merge_metadata_in_order(page_meta_map)

    # ✅ 清洗校准依据，去掉冒号后面的量程说明
    if "校准依据" in final_metadata_props:
        final_metadata_props["校准依据"] = clean_calibration_basis_list(final_metadata_props["校准依据"])

    # 按页码顺序合并表格行
    all_extracted_rows = []
    for p_num in sorted(page_results_map.keys()):
    ...
    # ✅ 补充 required keys（包含内页字段，确保就算没出现也保留 null）
    required_keys = [
    ...
    output_json_path = str(Path(out_dir) / Path(pdf_filename).with_suffix(".json"))
    with open(output_json_path, "w", encoding="utf-8") as f:
        json.dump(final_json, f, ensure_ascii=False, indent=2)

    print(f"✅ [Parser] 解析完成！JSON 已保存至: {output_json_path}")
