...
class Config:
    _app = get_app_config()
    TEMP_DB_DIR = _app.temperature_db_dir
...
# ========== 核验逻辑（修改版） ==========
def check_environment(json_file, config):
    ...
    print(f"🧠 正在加载语义模型：{app_config.embed_model_path}")
    from sentence_transformers import SentenceTransformer

    embed_model = SentenceTransformer(app_config.embed_model_path)

    collection = get_collection(app_config.temperature_db_dir, "temperature_data")
    ...
    for criterion in criteria_list:
        ...
        # 不限制实验室，保留所有匹配记录用于核验
        if not db_entries:
            report_lines.append(f"❌ {code} 在数据库中没有匹配记录")
            continue
